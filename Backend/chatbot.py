...
def get_chatbot_response(query: str) -> str:
    ...
    try:
        ...
        if not plant_type:
            return "I can only answer questions about the identified plant. Please make sure you've uploaded a plant image first."
...
def get_predefined_answer(query: str, plant_type: str) -> Optional[str]:
    """
    Fetch a predefined answer from the knowledge base for a query.
    ...
    Returns:
        Optional[str]: The predefined answer if found, or None
    """
    try:
        conn = sqlite3.connect("C:\\Users\\ASUS\\Documents\\MAJOR_PROJECT\\Backend\\plants.db")
        cursor = conn.cursor()

        # Make the search case-insensitive and plant-specific
        query = query.lower()
        cursor.execute(
            """
            SELECT answer FROM knowledge_base 
            WHERE LOWER(question) LIKE ? 
            AND LOWER(plant_type) = ?
            """,
            (f"%{query}%", plant_type.lower())
        )
        ...
        logger.error(f"Error in get_predefined_answer: {str(e)}")
        return None
    finally:
        if 'conn' in locals():
            conn.close()
